...
<body>
			...
			<form class="navbar-form " role="search" name="demography" action=""
				...
				<div class="form-group">
					<div style="padding-bottom: 10px">
						<label class="label label-info" for="keyword">Keyword</label> <input
							type="text" class="form-control" placeholder="Keyword" id="query"
							value="">
					</div>
					<div style="padding-bottom: 10px">
					...
					<div style="padding-bottom: 10px">
						<label class="label label-info" for="currentLocation">Nearby
							Address</label> <input type="text" class="form-control"
							placeholder="Current Location" id="zip">
					</div>
	...
	<script>
      ...
      function mapWithZip() {
        var url = "<%=request.getContextPath()%>/search/locationsearch?query=" + encodeURIComponent(qS).replace(/\%20/g, '%3d') + "&miles=" + miles + "&zip=" + encodeURIComponent(zip).replace(/\%20/g, '%3d');
        url += "&lat="+gLat+"&lng="+gLng;
      ...
      function begin(realLat, realLng) {
        ...
        realLng = gLng;
        zip = null;
        url = "<%=request.getContextPath()%>/search/locationsearch?query="
			...
			for (var i = 0; i < len; i++) {
				...
				var li = $('<p class = "list-group-item-text">').html(
						results[i].address + '</br>' + results[i].phone
								+ '</br> Rating: ' + results[i].rating + '</br>' + '<i>'
